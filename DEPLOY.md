...
make test        # 크레딧·결제 회귀 테스트
make prod-down   # 정지
make help        # 전체 타깃 목록
```

크레딧 차감·적립이나 결제 쪽 코드를 고쳤으면 배포 전에 `make test` 를 돌린다.
잔액이 음수로 내려가는지, 같은 결제가 두 번 적립되는지, 번역기가 죽었을 때
생성이 막히는지를 확인한다 (인메모리 SQLite 를 쓰므로 운영 DB 에 붙지 않고,
서비스가 떠 있는 중에 돌려도 안전하다).
