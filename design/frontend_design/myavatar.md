...
> 이 문서는 `Avatarbank`의 **사용자(Seller)**가 아바타를 등록하고 관리하는 프론트엔드 화면을 정의한다.  
...
**레이아웃 비율**: `My Avatars` : `Training Request` = `1 : 2`, 세로로 정렬

### 1.1 Training Request 영역
...
**동작**:
- 신규요청 버튼 클릭 시 → [2.1 학습요청 팝업](#21-아바타-학습-요청-팝업) (모달) 표시

### 1.2 My Avatars 영역

- **레이아웃**: 카드의 배열 형식(3개가 한줄, 2줄이상일시 스크롤)
- **아바타 카드 구성**:
  - 정사각형 영역
  - 대표 이미지
  - 아바타 이름
