...
class Settings(BaseSettings):
    ...
    # 예: "https://avatarclub.example.com,https://www.avatarclub.example.com"
    CORS_EXTRA_ORIGINS: str = ""

    # ------------------------------------------------------------------
    # 프롬프트 번역 (한국어 → 영어)
    # ------------------------------------------------------------------
    # z-image 는 영어 캡션으로 학습돼 한국어 프롬프트를 제대로 못 알아듣는다.
    # fal 로 나가기 직전에 영어로 바꾼다. "none" 이면 입력 그대로 보낸다.
    TRANSLATE_PROVIDER: str = "none"  # none | gemini
    # 번역 실패·지연은 생성을 막지 않고 원문으로 폴백한다. 그래도 앞단이 오래 끌면
    # 생성 전체가 Cloudflare 무료 플랜의 100초 제한에 먼저 걸리므로 짧게 잡는다.
